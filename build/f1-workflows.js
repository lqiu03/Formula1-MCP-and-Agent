...
async function analyzeSeasonStandings(season) {
    ...
    let consistentPerformers = new Set();
...
async function compareDriversAcrossSeason(season, driver1Number, driver2Number) {
    const meetings = await openF1Client.getMeetings(season);
    const comparison = {
        season,
        ...
        try {
            ...
            if (raceSession) {
                ...
                if (driver1Result && driver2Result) {
                    ...
                    if (driver1Result.position === 1)
                        comparison.driver1.wins++;
                    if (driver2Result.position === 1)
                        comparison.driver2.wins++;
                    if (driver1Result.position <= 3)
                        comparison.driver1.podiums++;
                    if (driver2Result.position <= 3)
                        comparison.driver2.podiums++;
                    // Head-to-head
...
export const championshipAnalysisWorkflow = createTool({
    ...
    execute: async ({ context: { season, includeDriverComparison = false, focusTeams = [] } }) => {
        try {
...
export const raceWeekendAnalysisWorkflow = createTool({
    ...
    execute: async ({ context: { year, location, includeWeatherImpact = false } }) => {
        try {
            console.log(`🔄 Analyzing race weekend: ${year} ${location}...`);
            // Step 1: Find the race
            const meetings = await openF1Client.getMeetings(year);
            const raceMeeting = meetings.find(m => m.meeting_name.toLowerCase().includes(location.toLowerCase()) ||
...
//# sourceMappingURL=f1-workflows.js.map
...
function formatRaceResults(results: Array<{ position: number; driver: any }>, title: string) {
...
function formatPodium(podium: Array<{ position: number; driver: any }>, raceInfo: string) {
...
server.registerTool(
  ...
  async ({ year, location }: { year: number; location: string }) => {
    try {
...
server.registerTool(
  ...
  async ({ year, location, top_n }: { year: number; location: string; top_n?: number }) => {
    try {
      // Find the meeting
...
server.registerPrompt(
  ...
  ({ year, location }: { year: string; location: string }) => ({
    messages: [{
      role: "user",
      content: {
        type: "text",
...
server.registerPrompt(
  ...
  ({ year1, location1, year2, location2 }: { year1: string; location1: string; year2: string; location2: string }) => ({
    messages: [{
      role: "user",
      content: {
        type: "text",

...
export class OpenF1Client {
  ...
  async getSessions(meetingKey: number): Promise<Session[]> {
    return this.fetchData<Session>('sessions', { meeting_key: meetingKey });
  }

  /**
   * Get the race session for a meeting (session_type = 'Race')
   */
  async getRaceSession(meetingKey: number): Promise<Session | null> {
  ...
  async getPositions(sessionKey: number): Promise<Position[]> {
    return this.fetchData<Position>('position', { session_key: sessionKey });
  }
  ...
  async getDrivers(sessionKey: number): Promise<Driver[]> {
    return this.fetchData<Driver>('drivers', { session_key: sessionKey });
  }

  /**
   * Get race results with driver information
   */
  async getRaceResults(sessionKey: number): Promise<Array<Position & { driver: Driver }>> {
    ...
    const results: Array<Position & { driver: Driver }> = [];
   ...
   */
  async getPodiumWinners(year: number, location: string): Promise<Array<Position & { driver: Driver }> | null> {
    try {
      // Find the meeting

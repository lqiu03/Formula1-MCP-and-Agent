...
export const f1AnalysisTools = {
  analyzeDriverPerformance: createTool({
    ...
    execute: async ({ context: { driverName, season, circuits } }: { context: { driverName: string; season: number; circuits?: string[] } }) => {
      try {
        ...
        for (const meeting of meetings) {
          ...
          if (circuits && circuits.length > 0) {
            const circuitMatch = circuits.some(circuit => 
              meeting.location.toLowerCase().includes(circuit.toLowerCase()) ||
          ...
          const driverResult = results.find(r => 
            r.driver.full_name.toLowerCase().includes(driverName.toLowerCase()) ||
  ...
  compareDrivers: createTool({
    ...
    execute: async ({ context: { driver1, driver2, season, metric } }: { context: { driver1: string; driver2: string; season: number; metric: "wins" | "podiums" | "points" | "head_to_head" } }) => {
      try {
        const meetings = await openF1Client.getMeetings(season);
        
        let comparison = `🆚 **Driver Comparison: ${driver1} vs ${driver2} (${season})**\n\n`;
        
        const driver1Stats = { wins: 0, podiums: 0, points: 0, positions: [] as number[] };
        const driver2Stats = { wins: 0, podiums: 0, points: 0, positions: [] as number[] };
        ...
        for (const meeting of meetings) {
          ...
          if (d1Result && d2Result) {
            ...
            if (d1Result.position === 1) driver1Stats.wins++;
            if (d1Result.position <= 3) driver1Stats.podiums++;
            ...
            if (d2Result.position === 1) driver2Stats.wins++;
            if (d2Result.position <= 3) driver2Stats.podiums++;
  ...
  raceWeatherAnalysis: createTool({
    ...
    execute: async ({ context: { year, location } }: { context: { year: number; location: string } }) => {
      try {
